 ...
 * 登录界面的自定义editText
 */
public class MyNewLoginEditText extends LinearLayout implements View.OnFocusChangeListener, View.OnClickListener {
    ...
    public MyNewLoginEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        rootView = LayoutInflater.from(context).inflate(R.layout.view_mylogin_edit_text2, this);
        findViews(rootView);
        init(attrs);
    }
...
//        edit_content.setFocusable(isEdit);
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        if (hasFocus) {
            view_line.setBackgroundResource(R.color.global_popular_color);
        } else {

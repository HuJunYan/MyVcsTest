...
import com.maibai.user.model.ConsumeDataBean;
import com.maibai.user.model.ConsumeSmallBillBean;
import com.maibai.user.model.InstallmentHistoryBean;
...
import com.maibai.user.model.WithdrawalsBillItemBean;
import com.maibai.user.net.api.OrderQuery;
import com.maibai.user.net.api.Repayment;
...
import com.maibai.user.utils.CashBillListUtil;
...
public class RepayPasswordActivity extends BaseActivity implements View.OnClickListener {
        ...
        try {
            ...
            mRepayment.repayment(mJson, v, true,type,  new BaseNetCallBack<ResponseBean>() {
                        ...
                        public void onSuccess(ResponseBean mResponseBean) {
                            switch (from) {
                                case GlobalParams.REPAY_FROM_SHOUFU:
                                    UserUtil.setStatus(mContext, GlobalParams.WAIT_FOR_PAY);
                                    putOrder();
                                    break;
                                case GlobalParams.REPAY_FROM_CONSUMPTION:
    ...
    private void putOrder() {
        try {
            ...
            jsonObject.put("consume_id", mBundle.getString("consume_id"));
            new OrderQuery(mContext).orderQuery(jsonObject, null, true, new BaseNetCallBack<OrderRealPayBean>() {
                ...
                public void onSuccess(OrderRealPayBean paramT) {
                    if ("2".equals(paramT.getData().getState())) {
                        ...
                        mBundle.putSerializable(GlobalParams.ORDER_BEAN_KEY, Utils.orderRealPayBean2OrderBean(paramT));
                        mBundle.putBoolean("isSuccess", true);
                        switch (paramT.getData().getRepay_type()) {
                            case GlobalParams.NEXT_MONTH_REPAY:
                                gotoActivity(mContext, PaySucNextMonthActivity.class, mBundle);
                                backActivity();
                                break;
                            case GlobalParams.FENQI_REPAY:
                                gotoActivity(mContext, PaySuccessActivity.class, mBundle);
                                backActivity();
                                break;
                        }
                    }
                ...
                @Override
                public void onFailure(String url, int errorType, int errorCode) {
                    mBundle.putBoolean("isSuccess", false);
                    mBundle.putString("error_result", url);
                    gotoActivity(mContext, PaySucNextMonthActivity.class, mBundle);
                    backActivity();
                }
            });

...
import com.maibai.user.model.WeChatOrder;
import com.maibai.user.net.api.GetWeChatOrder;
        ...
        try {
            ...
            new OrderQuery(mContext).orderQuery(jsonObject, null, true, new BaseNetCallBack<OrderRealPayBean>() {
                ...
                public void onSuccess(OrderRealPayBean paramT) {
                    if(null==paramT){
                        ToastUtil.showToast(mContext,"您已付款，数据异常未完成订单");
                        return;
                    }
                    ...
                    if(null==paramT.getData().getState()){
                        ToastUtil.showToast(mContext,"您已付款，数据异常未完成订单");
                        return;
                    }
                    if ("2".equals(paramT.getData().getState())) {
                        ...
                        String repayType=paramT.getData().getRepay_type();
                        if(null==repayType){
                            ToastUtil.showToast(mContext,"您已付款，数据异常未完成订单");
                            return;
                        }

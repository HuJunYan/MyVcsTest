...
public class OrderInstallmentRepay extends NetBase {
    private boolean isRelease = true;
    private String mUrl;
    private Context mContext;
    private JSONObject mJSONObject;

    public OrderInstallmentRepay(Context context) {
        super(context);
        mContext = context;
        mUrl = NetConstantValue.getOrderInstallmentRepayUrl();
    }

    public void orderInstallmentRepay(JSONObject jsonObject, final BaseNetCallBack<OrderBean> mOrderCallBack) {
        try {
    ...
    public void orderInstallmentRepay(JSONObject jsonObject, View view, boolean isShowDialog, final BaseNetCallBack<OrderBean> mOrderCallBack) {
        try {
            jsonObject.put("pay_pass", Utils.MD5SHA1AndReverse(jsonObject.getString("pay_pass")));
            mJSONObject = SignUtils.signJsonNotContainList(jsonObject);
            if (mJSONObject == null) {
    ...
    private OrderBean test() {
        ...
        mOrderBean.setMsg("OrderInstallmentRepay in success");
        mOrderBean.getData().setAmount("35000");
        mOrderBean.getData().setRepay_type("2");
        long repayDate = System.currentTimeMillis() / 1000 + 24 * 60 * 60L * 30;
        mOrderBean.getData().setRepay_date(repayDate + "");
        mOrderBean.getData().setRepay_times("6");
        mOrderBean.getData().setRepay_principal("5833");
        mOrderBean.getData().setRepay_interest("1100");

...
public class SetPayPassword extends NetBase {
    private boolean isRelease = true;
    private String mUrl;
    private Context mContext;
    private JSONObject mJSONObject;

    public SetPayPassword(Context context) {
        super(context);
        mContext = context;
        mUrl = NetConstantValue.getSetPayPassUrl();
    }

    public void setPayPassword(JSONObject jsonObject, final BaseNetCallBack<ResponseBean> mResponseCallBack) {
        try {
    ...
    public void setPayPassword(JSONObject jsonObject, View view, boolean isShowDialog, final BaseNetCallBack<ResponseBean> mResponseCallBack) {
        try {
            jsonObject.put("pay_pass", Utils.MD5SHA1AndReverse(jsonObject.getString("pay_pass")));
            mJSONObject = SignUtils.signJsonNotContainList(jsonObject);
            if (mJSONObject == null) {
    ...
    private ResponseBean test() {
        ...
        mResponseBean.setMsg("setPayPass in success");
